...
This tutorial introduces basic ReSift concepts through building an app called _ReSift Rentals_. This app lets users to browse movies and update movie information. Click here to see the [completed ReSift Rentals app](https://gtcpo.csb.app/). It has the following functionalities:

- It fetches genre data and presents each genre’s name and the thumbnails of the movies in them.
- It optimizes performance by only fetching the data _when needed/in batches_.
  - In the initial load, it fetches 10 movies for each genre to show their movie thumbnails. It’ll fetch the next batch of 10 movies when the user scrolls past the current batch.
...
A few functionalities in the app were implemented with the help of third party libraries, which we’ll introduce them when they are being used, you do not need prior knowledge about them.
